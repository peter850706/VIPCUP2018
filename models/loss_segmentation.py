...
class LossFunction(Layer):
    def __init__(self,
        ...
        if self._data_loss_func.__name__.startswith('cross_entropy'):
            ...
            self._softmax = False
        ### Focal loss
        if self._data_loss_func.__name__ == 'focal_loss': 
            tf.logging.info('Focal loss function always performs sigmoid internally.')
            self._softmax = False
...
def tversky(prediction, ground_truth, weight_map=None, alpha=0.3, beta=0.7):
    """
...
def focal_loss(prediction, ground_truth, weight_map=None):
    """
    Function to calculate the focal loss function (reference: https://arxiv.org/pdf/1708.02002.pdf)
    :param prediction: the logits (before softmax)
    :param ground_truth: the segmentation ground truth
    :param weight_map:
    :return: the focal loss
    """
    # weighting factor ALPHA and focusing parameter GAMMA
    ALPHA = 0.75
    GAMMA = 2.0
    ...
    p_t = tf.sparse_reduce_sum(one_hot * tf.nn.sigmoid(prediction), axis=[-1])
    tmp0 = tf.fill(ground_truth.shape, 1.0 - ALPHA)
    tmp1 = tf.fill(ground_truth.shape, ALPHA)
    tmp = tf.stack([tmp0, tmp1], axis=-1)
    ALPHA_t = tf.sparse_reduce_sum(one_hot * tmp, axis=[-1])
    loss = -ALPHA_t * tf.pow((1.0 - p_t), GAMMA) * tf.log(tf.clip_by_value(p_t, 1e-37, 1.0))
    
    if weight_map is None:
        return tf.reduce_mean(loss)

    weight_sum = tf.maximum(tf.reduce_sum(weight_map), 1e-6)
    return tf.reduce_mean(loss * weight_map / weight_sum)